...
#AKBgoupのメンバーのアカウント取得
# 名前,talentinstaのURL,アイコン画像URL,オフィシャルURL


# http://www.talentinsta.com/matome/index.php?p=AKB48
id = 0
talentUrls = []
rows = []

Anemone.crawl("http://www.talentinsta.com/matome/index.php?p=AKB48",:depth_limit => 0) do |anemone|
...
Anemone.crawl("http://www.talentinsta.com/matome/index.php?p=SKE48",:depth_limit => 0) do |anemone|
    anemone.on_every_page do |page|
        doc = Nokogiri::HTML.parse(page.body.toutf8)
        body = doc.xpath('//a').each do |node|
            str = node.text
...
Anemone.crawl("http://www.talentinsta.com/matome/index.php?p=JKT48",:depth_limit => 0) do |anemone|
    anemone.on_every_page do |page|
        doc = Nokogiri::HTML.parse(page.body.toutf8)
        body = doc.xpath('//a').each do |node|
            str = node.text
...
Anemone.crawl("http://www.talentinsta.com/matome/index.php?p=SDN48",:depth_limit => 0) do |anemone|
    anemone.on_every_page do |page|
        doc = Nokogiri::HTML.parse(page.body.toutf8)
        body = doc.xpath('//a').each do |node|
            str = node.text
